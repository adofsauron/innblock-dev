...
class alllist
{
 ...
 public:
  ...
  void slot_add(slotinfo slot) { slotlist.push_front(slot); }

  void rec_add(recinfo rec) { reclist.push_back(rec); }

  void del_add(recinfo rec) { dellist.push_back(rec); }
  ...
  void printrec()
  {
    ...
    for (itlogic = reclist.begin(); itlogic != reclist.end(); ++itlogic)
    {
      ...
      if (itlogic->record_type == REC_STATUS_INFIMUM)
      {
        printf("(%d) INFIMUM record offset:%d heapno:%lu n_owned %lu,delflag:%c minflag:%lu rectype:%lu\n", cou,
               itlogic->offset, itlogic->heap_no, (ulint)(itlogic->n_owned), is_del(itlogic->delflag),
               (ulint)(itlogic->minflag), (ulint)(itlogic->record_type));
      }
    ...
    for (itphy = phylist.begin(); itphy != phylist.end(); ++itphy)
    {
      ...
      {
        printf("(%d) INFIMUM record offset:%d heapno:%lu n_owned %lu,delflag:%c minflag:%lu rectype:%lu\n", cou,
               itphy->offset, itphy->heap_no, (ulint)(itphy->n_owned), is_del(itphy->delflag), (ulint)(itphy->minflag),
               (ulint)(itphy->record_type));
      }
    ...
    for (it = dellist.begin(); it != dellist.end(); ++it)
    {
      cou++;
      printf("(%d) normal record offset:%d heapno:%lu n_owned %lu,delflag:%c minflag:%lu  rectype:%lu\n", cou,
             it->offset, it->heap_no, (ulint)(it->n_owned), is_del(it->delflag), (ulint)(it->minflag),
             (ulint)(it->record_type));
    }
  }
};
